...
const AdminPanel = () => {
  ...
  const [activeSection, setActiveSection] = useState<"overview" | "manage-users" | "ongoing-trips" | "users-overview" | "trip-analysis" | "insights">("overview");
  ...
  const sectionConfig = [
    { id: "manage-users" as const, label: "Manage Users", icon: Users },
    { id: "ongoing-trips" as const, label: "Recent Ongoing Trips", icon: Activity },
    { id: "users-overview" as const, label: "Total Users Data", icon: BarChart3 },
    { id: "trip-analysis" as const, label: "Analysis of Every Trip", icon: TrendingUp },
    { id: "insights" as const, label: "Operational Insights", icon: Shield },
  ];

  return (
    <div className="flex min-h-screen gap-6 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 p-4">
      {/* Sidebar Navigation */}
      <motion.aside initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="w-64 flex-shrink-0">
        <div className="sticky top-4 rounded-3xl border border-slate-200 bg-white/60 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-white/10 p-6 space-y-6">
          {/* Branding */}
          <div className="text-center">
            <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-fuchsia-500 text-white font-black mb-2">
              T
            </div>
            <p className="text-sm font-black text-slate-900 dark:text-white">Traveloop</p>
          </div>

          {/* Analytics Overview Button */}
          <button
            onClick={() => setActiveSection("overview")}
            className={`w-full rounded-2xl px-4 py-3 font-bold transition flex items-center justify-center gap-2 ${
              activeSection === "overview"
                ? "bg-gradient-to-r from-indigo-500 to-fuchsia-500 text-white shadow-lg"
                : "border border-slate-200 bg-white/50 text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-white/80 hover:bg-white/70"
            }`}
          >
          ...
          <div className="space-y-2">
            {sectionConfig.map((section) => (
              <button
                ...
                className={`w-full rounded-xl px-4 py-2.5 text-sm font-semibold transition flex items-center gap-3 ${
                  activeSection === section.id
                    ? "bg-slate-200/80 text-slate-900 dark:bg-white/20 dark:text-white"
                    : "text-slate-600 dark:text-white/60 hover:bg-slate-100/50 dark:hover:bg-white/5"
                }`}
              >
          ...
          <button
            ...
            onClick={() => load(true)}
            disabled={refreshing}
            className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white/60 px-3 py-2 text-xs font-bold text-slate-700 transition hover:bg-white dark:border-white/10 dark:bg-white/10 dark:text-white/80"
          >
      ...
      <div className="mx-auto max-w-6xl space-y-8 py-4">
        ...
        {/* Section 2: Recent Ongoing Trips */}
        {activeSection === "ongoing-trips" && (
          <section className="space-y-4">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Recent Ongoing Trips</h2>
        ...
        {/* Section 3: Total Users Data Overview */}
        {activeSection === "users-overview" && (
          <section className="space-y-4">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Total Users Data Overview</h2>
        ...
        {/* Section 4: Analysis of Every Trip */}
        {activeSection === "trip-analysis" && (
          <section className="space-y-4">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Analysis of Every Trip</h2>
        ...
        {activeSection === "insights" && (
          <section className="space-y-4">
            <h2 className="text-2xl font-black text-slate-900 dark:text-white">Operational Insights</h2>
            <div className="grid gap-4 md:grid-cols-3">
